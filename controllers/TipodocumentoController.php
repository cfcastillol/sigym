...
class TipodocumentoController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save()){
                Yii::$app->session->setFlash('success', 'Tipo de documento almacenado con exito');
            }else{
                Yii::$app->session->setFlash('danger', 'No se pudo crear el tipo de documento');
            }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save()){
                Yii::$app->session->setFlash('success', 'Tipo de documento actualizado con exito');
            }else{
                Yii::$app->session->setFlash('danger', 'No se pudo actualizar el tipo de documento');
            }
            return $this->redirect(['index']);
        } else {
