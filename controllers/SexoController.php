...
class SexoController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save()){
                Yii::$app->session->setFlash('success', 'Sexo almacenado con exito');
            }else{
                Yii::$app->session->setFlash('danger', 'No se pudo crear el sexo');
            }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save()){
                Yii::$app->session->setFlash('success', 'Sexo Actualizado con exito');
            }else{
                Yii::$app->session->setFlash('danger', 'No se pudo actualizar el sexo');
            }
            return $this->redirect(['index']);
        } else {
